...
/*
  App lengkap (fixed)
  - Ganti src/App.js dengan file ini
  - Pastikan file aset (JADI.png, gojo1.jpg, special.jpg) berada di /public
  - npm install framer-motion
*/

// -- Assets (ganti sesuai kebutuhan) ---------------------------------
const GOJO_SRC = "/JADI.png"; // atau URL eksternal
const SURPRISE_MEDIA = "/gojo1.jpg"; // file video atau gambar di public/
...
// Inject the merged CSS into document head (once)
const MERGED_CSS = `
      ...
      <div className="balloon-layer" aria-hidden>
        {balloons.map((b) => (
          <div
            ...
            style={{ left: b.left, animationDelay: b.delay, transform: `scale(${b.scale})` }}
          />
        ))}
      </div>
...
function Surprise({ onRestart }) {
  // gunakan MediaPreview agar component dipakai (menghilangkan unused-vars)
  return (
    <motion.div className="page surprise final" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <h1 className="glitter-text">Selamat Ulang Tahun, KATA TETEH JANGAN GALAK GALAK!</h1>
      <h1>😂</h1>

      <div className="final-media" style={{ display: 'flex', gap: 18, alignItems: 'center', justifyContent: 'center', flexWrap: 'wrap' }}>
        <MediaPreview src={SURPRISE_MEDIA} style={{ maxWidth: 420 }} />
      </div>
